...
`ago news` 系列命令专门订阅别人的 RSS 消息。关于阅读自己的微博客请看上面的 `ago tl` 命令。
...
- `ago tl -fav --yday` 阅读昨天的全部消息 (限定收藏消息)
- `ago tl -fav -date [date]` 阅读指定日期的全部消息 (限定收藏消息)
- `ago tl -fav -count [date]` 统计指定日期的消息条数 (限定收藏消息)


## Tag and Search (标签与搜索)

标签必须以“空格井号”开头，以空格结尾，并且不超过 TagSizeLimit

ago search [keyword], 如果 keyword 以井号开头，则先自动采用标签搜索模式，找不到再改成普通搜索模式。

`ago search [keyword] -fav`
`ago search --tag [tag]` 由于在命令行输入井号不方便
